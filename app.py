...
def get_openai_client():
    ...
    if not api_key:
        ...
        st.info("🔧 배포 환경에서는 Secrets에 OPENAI_API_KEY를 추가해주세요.")
        st.stop()
...
# OpenAI API 함수들
# ---------------------------
def call_openai_api(messages, max_tokens=500, temperature=0.7):
    """OpenAI API 호출 함수"""
    try:
...
def generate_ai_suggestions(context, block, n=3):
    ...
    # 캐시 키 생성
    cache_key = f"{block}_{hash(str(context))}"
...
- 책 내용: {context.get('book', '내용 없음')[:500]}
...
"""
    ...
    with st.spinner("🤖 AI가 제안을 만들고 있어요..."):
        response = call_openai_api(messages, max_tokens=400, temperature=0.8)
...
"""
    ...
    messages = [{"role": "user", "content": prompt}]
    response = call_openai_api(messages, max_tokens=300, temperature=0.7)
...
"""
    ...
    messages = [{"role": "user", "content": prompt}]
    response = call_openai_api(messages, max_tokens=300, temperature=0.3)
...
def render_sidebar():
    ...
    if selected_book != "직접 입력":
        ...
        st.session_state["book_text"] = f"{book_info['summary']}\n\n주요 장면들:\n" + "\n".join([f"- {scene}" for scene in book_info['key_scenes']])
    ...
    st.sidebar.divider()
    if st.sidebar.button("📊 활동 로그 다운로드"):
        data = {
...
def render_suggestion_block(label, key_block, icon):
    ...
    if st.button(f"🤖 AI {label} 제안 받기", key=f"generate_{key_block}"):
        ctx = {
            "book_title": st.session_state["book_title"],
            "book": st.session_state["book_text"],
            "outline": st.session_state["outline"],
            "draft": st.session_state["draft"],
        }
            ...
            with st.container(border=True):
                ...
                with c1:
                    if st.button(f"✅ 선택", key=f"accept_{key_block}_{i}"):
                        ...
                        addition = f"\n\n{sug}" if current_draft.strip() else sug
                        st.session_state["draft"] += addition
